...
void read_reenc_file( char* file,    GByteArray** rcp_buf,
											int* file_len, GByteArray** aes_buf )
{
	FILE* f;
	int i;
	...
	*file_len = 0;
	...
	for( i = 3; i >= 0; i-- )
		len |= fgetc(f)<<(i*8);
	g_byte_array_set_size(*rcp_buf, len);
	fread((*rcp_buf)->data, 1, len, f);
...
void
write_reenc_file( char* file,   GByteArray* rcp_buf,
									int file_len, GByteArray* aes_buf )
{
	FILE* f;
	int i;
	...
	/* write rcp_buf */
	for( i = 3; i >= 0; i-- )
		fputc((rcp_buf->len & 0xff<<(i*8))>>(i*8), f);
	fwrite(rcp_buf->data, 1, rcp_buf->len, f);
