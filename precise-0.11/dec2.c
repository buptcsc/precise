...
char* usage =
"Usage: precise-dec2 [OPTION ...] PUB_KEY PRIV_KEY FILE\n"
"\n"
...
/* 		} */
	...
	{
		if(  strlen(in_file) > 8 &&
				!strcmp(in_file + strlen(in_file) - 8, ".enc.rec") )
			out_file = g_strndup(in_file, strlen(in_file) - 8);
		else
...
main( int argc, char** argv )
{
	...
	read_reenc_file(in_file, &rcp_buf, &file_len, &aes_buf);

	rcp = bswabe_rcp_unserialize(pub, rcp_buf, 1);
	if( !bswabe_dec2(pub, prv, identity, rcp, m) )
		die("%s", bswabe_error());
	bswabe_rcp_free(rcp);
